...
namespace M3D_ISICG
{
	...
	Texture TriangleMeshModel::_loadTexture( const aiString & p_path, const std::string & p_type )
	{
		...
		if ( image.load( fullPath ) )
		{
			...
			int mipmapLevels = log2( glm::max( image._width, image._height ) ); //Calculez le nombre de niveaux de mipmaps necessaires
			...
			glTextureParameteri(texture._id,GL_TEXTURE_MIN_FILTER,GL_LINEAR_MIPMAP_LINEAR ); // Un filtre de Minification. Nous allons utiliser la m´ethode du mipmapping

			glTextureParameteri( texture._id, GL_TEXTURE_MAG_FILTER, GL_LINEAR ); // filtrage bilinéaire

			

			// Fill the texture.
			glTextureSubImage2D(texture._id, 0, 0, 0, image._width, image._height, format, GL_UNSIGNED_BYTE, image._pixels );
			glGenerateTextureMipmap( texture._id );//Creez la mipmap
		}
